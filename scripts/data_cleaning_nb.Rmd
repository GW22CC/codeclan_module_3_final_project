...
- have retained "Percent Of Adults" initially in case joing other dfs

```{r}
glimpse(shs_aggregate_responses)
```
...
```
```{r}
names(shs_proportions_data_join)
```


```{r}
names(shs_aggregate_responses_data)
...
shs_proportions_data_clean <- shs_proportions_data_join # re-assigning cleaned data to explicity object
